      ...
      {tabs.map(tab => (
        <button
          ...
          className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 font-medium ${
            activeTab === tab.id 
              ? 'bg-[var(--accent)]/10 text-[var(--accent)] shadow-sm' // Active: Subtle Tint
              : 'text-[var(--text-secondary)] hover:bg-[var(--bg-secondary)] hover:text-[var(--text-primary)]' // Inactive: Ghost
          }`}
        >
